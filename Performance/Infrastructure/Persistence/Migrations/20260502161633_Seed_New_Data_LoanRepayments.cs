...
namespace Performance.Migrations
{
    ...
    public partial class Seed_New_Data_LoanRepayments : Migration
    {
        ...
        private static string BuildBatchSql(int startId, int endId) => $@"
            IF NOT EXISTS (
                SELECT 1 FROM [Performance].[LoanRepayments] LR
                INNER JOIN [Performance].[Loans] L      ON L.[Id]  = LR.[LoanId]
                INNER JOIN [Performance].[Users] U      ON U.[Id]  = L.[UserId]
                WHERE U.[Username] = N'user{startId}'
                AND LR.[CreatedBy] = 'data seeding'
            )
            BEGIN
                WITH
                E2(N) AS (
                ...
                INSERT INTO [Performance].[LoanRepayments]
                    ([ScheduledDate], [ActualPaymentDate], [ScheduledAmount],
                    [PaidAmount], [LoanId], [CreatedAt], [CreatedBy], [UpdatedAt], [UpdatedBy])
                SELECT
                ...
                INNER JOIN [Performance].[Loans] L ON L.[UserId]   = U.[Id];
            END
            ";
    }
}